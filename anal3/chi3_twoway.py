...
# 변인이 두 개 - 독립성 또는 동질성 
# 독립성(관련성) 검정
# - 동일 집단의 두 변인 학력수준과 대학진학 여부를 대상으로 관련성이 있는
# - 독립성 검정은 두 변수 사이의 연관성을 검정한다.

# 실습 : 교육수준과 흠연율 간의 관련성 분석 smoke csv
# 대립: 교육수준과 흡연율 간의 관련이 없다.(독립이다, 연관성이 없다.)
...
# ab = pd.crosstab(index=data['education'], columns=data['smoking'], normalize=True)
print(ctab)
# print(ab)
...
# 결론 : p-value(0.000818257) < 0.05 이므로 기무가설 기각.
# 따라서 교육수준과 흠연율 간의 관련이 있다.
# .....

print('음료 종류와 성별 간의 선호도 차이 검정 -----------')
# 남성과 여성의 음료 선호는 서로 관련이 있을까?
# 귀무(H1): 성별과 음료 선호는 서로 관련이 없다. (성별에 따라 선호가 같다)
# 대립(H1): 성별과 음료 선호는 서로 관련이 있다. (성별에 따라 선호가 다름)
...
"""
# 결론: p값이 0.05 작으므로 귀무 기각. 즉, ...

# 시각화: heatmap
# 히트맵은 색상을 활용해 값의 분포를 보여주는 그래프.
# 히스토스램이 하나의 변수에 대한 강도를 활용할 수 있다면,
# 컬러맵은 색상을 활용해 두개의 기준(x축+y축)에 따른 강도(색상)을 보여준다고 생각하면 됨
